...
observeEvent(input$addAuthor, {
  ...
  author_values$dList <- c(isolate(author_values$dList), new_author)
})

# Render table of authors
output$author_table <- renderTable({author_values$df}, include.rownames = F)
...
```{r}
# Generate a new reactive list
exp <- reactiveValues()
exp$values <- list()

# On save add all inputs to a list
...
output$list<-renderPrint({ author_values$dList })
