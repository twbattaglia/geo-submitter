...
```{r save experiment}
h4("Help")
p("This section is the heart of", tags$b('GEO-submitter '), "and should be filled completely and as detailed as possible. The information stored here is meant for researchers to understand how the experiemental design as well as the platform and protocols used during sample extraction and treatment.")
...
```{r}
output$expReview <- renderTable(rownames = F, 
                                hover = T, {
  ...
  out <- data.frame(Variable = row.names(df), Values = df[,1])
  out_filter <- subset(out, !(Variable %in% c("authorDF.First", "authorDF.Middle", "authorDF.Last")))
  out_filter
})
