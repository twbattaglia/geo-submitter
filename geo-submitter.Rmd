...
actionButton(inputId = "saveExp", icon = icon("fa-list"), label = "Save", width = "215px")
...
textareaInput(inputId = "extraction", 
          label = "Describe the protocol used to isolate the extract material. You can include as much text as you need to thoroughly describe the protocol; it is strongly recommended that complete protocol descriptions are provided within your submission.", 
...
textareaInput(inputId = "hybridization", 
          label = "Describe the protocols used for hybridization, blocking and washing, and any post-processing steps such as staining. You can include as much text as you need to thoroughly describe the protocol.", 
          value = "", 
          placeholder = "Enter text...")
...
actionButton(inputId = "saveSample", icon = icon("fa-list"), label = "Save", width = "215px")
...
output$downloadData <- downloadHandler(
  ...
  # sampleInfo <- sampleInfo[!is.null(sampleInfo$SampleID), ] # Temporary fix to remove NA rows (Issue #2)
    for (i in 1:nrow(sampleInfo)){
      ...
      # Write !Sample_label
      write_append(paste0("!Sample_label", " = ", isolate(reactive$organism)), filename = file)
      ...
      write_append(paste0("!Sample_extract_protocol", " = ", isolate(reactive$organism)), filename = file)
      
      # Write !Sample_label_protocol
      write_append(paste0("!Sample_label_protocol", " = ", isolate(reactive$organism)), filename = file)
      
      # Write !Sample_hyb_protocol
      write_append(paste0("!Sample_hyb_protocol", " = ", isolate(reactive$organism)), filename = file)
      
      # Write !Sample_scan_protocol
      write_append(paste0("!Sample_scan_protocol", " = ", isolate(reactive$organism)), filename = file)
      
      # Write !Sample_data_processing
      write_append(paste0("!Sample_data_processing", " = ", isolate(reactive$organism)), filename = file)
      
      # Subset the expression values
      nsn_sub <- subset(nsn, select = i)
