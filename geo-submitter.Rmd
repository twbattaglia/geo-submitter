...
selectInput(inputId = 'dataType', 
            label = helpText("Select an array platform."), 
            choices = c("Select", 'Nanostring', "Microarray"), 
            selectize = T)
tags$hr()
...
fileInput(inputId = 'upload', 
          label = helpText("Use 'Shift' to highlight multiple files."), 
          multiple = T,
          accept = c(
            '.rcc',
            '.rcc.gz', 
...
data <- reactive({
  ...
  if(input$dataType == "Nanostring"){
    ...
    showNotification("Data processed...", type = "message")
    ...
    # Normalize based on 
    if (input$normalization == "RMA"){
      showNotification("Normalizing Data...", type = "warning")
      normalized <- rma(ab)
      geneN <- nrow(normalized@featureData@data)
      
    } else if (input$normalization == "MAS5"){
      showNotification("Normalizing Data...", type = "warning")
      normalized <- mas5(ab)
      geneN <- nrow(normalized@featureData@data)
      
    } else if (input$normalization == "GCRMA"){
      showNotification("Normalizing Data...", type = "warning")
      normalized <- gcrma(ab)
      geneN <- nrow(normalized@featureData@data)
    }
...
output$normalized <- renderHighchart({
  ...
  long_format <- reshape2::melt(data()$normalized, id.vars = c("Name")) 
